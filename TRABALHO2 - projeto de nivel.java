...
public class TRABALHO2{
  public static void main (String args[]){
    ...
    Calendar cal = Calendar.getInstance();//chamando relogio windows
    int tempoinicial = cal.get(Calendar.SECOND);
    int tempomediano = tempoinicial - 1;
    int tempojogo = 0;
    int dificuldade = 0;
    
    System.out.println("ESCOLHA UMA DIFICULDADE ENTRE MAIOR QUE 1, mas cuidado, pode se tornar insano!");
    ...
    while (jogando){     
      ...
      //PROTECAO PARA SEGUNDOS = 00
      if (tempoinicial == 0){
        tempoinicial = 1;
        tempomediano = tempoinicial + 1;
      }
      
      cal = Calendar.getInstance();
      tempoinicial = cal.get(Calendar.SECOND); 
      
      
      
      //DESLOCAMENTO VERTICAL
      if (tempoinicial == tempomediano + 1){
        tempojogo++;
        tempomediano++;
        for(int Ind1 = 0 ; Ind1 < enem.length ; Ind1++){
          if (enem[Ind1] != null){
            enem[Ind1].y = enem[Ind1].y + dificuldade;
          }
        }
      } 
